...
bool Mmap::init(const char *filepath)
{
...
bool Mmap::get_record(const std::vector<uint8_t> &wire_name, int32_t qtype, std::vector<DNSResponseData> &out_records)
{
    out_records.clear();
...
bool Mmap::append_record(const std::vector<uint8_t> &wire_name, int32_t qtype, uint32_t ttl, uint16_t priority, const std::vector<uint8_t> &binary_rdata)
{
...
bool Mmap::delete_record(const std::vector<uint8_t> &wire_name, int32_t qtype)
{
    uint64_t hash = calculate_hash(wire_name);
    size_t bucket_idx = find_bucket(hash, qtype);
...
uint64_t Mmap::calculate_hash(const std::vector<uint8_t> &wire_name) const
{
...
void Mmap::push_free_slot(int32_t slotidx)
{
...
size_t Mmap::find_bucket(uint64_t hash, int32_t qtype) const
{
